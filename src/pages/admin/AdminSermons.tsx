...
import { Plus, Book, Trash2, Pencil } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { supabase } from "@/integrations/supabase/client";

const emptyForm = { title: "", speaker: "Bishop Paul Ndolo Mulu", date: "", video_url: "", topic: "" };
...
const AdminSermons = () => {
  ...
  const handleEdit = (s: any) => {
    setForm({ title: s.title, speaker: s.speaker || "", date: s.date, video_url: s.video_url || "", topic: s.topic || "" });
    setEditId(s.id);
    setDialogOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setEditId(null); setForm(emptyForm); } }}>
          <DialogTrigger asChild><Button onClick={openAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground"><Plus className="h-4 w-4 mr-2" />Add Sermon</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleSave} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">{editId ? "Update Sermon" : "Add Sermon"}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
